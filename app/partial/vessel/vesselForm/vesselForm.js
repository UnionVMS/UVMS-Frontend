 ...
 */
angular.module('unionvmsWeb').controller('VesselFormCtrl',function($scope, $log, $uibModal, $filter, Vessel, VesselContact, VesselNotes, vesselRestService, alertService, locale, mobileTerminalRestService, confirmationModal, GetListRequest, userService, configurationService, MobileTerminalHistoryModal, MobileTerminal, $q) {
    ...
    $scope.setDefaultCountryCode = function(){
        $scope.flagStateCode  = configurationService.getValue('VESSEL_PARAMETERS', 'ASSET.DEFAULT.FLAGSTATE');
        if(angular.isDefined($scope.flagStateCode)){
            $scope.vesselObj.flagStateCode = $scope.flagStateCode;
        }
    };
